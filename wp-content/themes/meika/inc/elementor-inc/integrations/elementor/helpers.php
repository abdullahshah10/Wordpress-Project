...
function meika_el( $className = null ) {
	if ( $className && class_exists( "meika{$className}" ) ) {
		if ( ( $cls = "meika{$className}" ) && method_exists( $cls, 'instance' ) ) {
			return $cls::instance();
		}
	}
	return meika_Hm::get_instance();
}